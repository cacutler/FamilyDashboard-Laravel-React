...
const breadcrumbs: BreadcrumbItem[] = [{title: 'To-Dos', href: '/todos'}, {title: 'To-Do', href: '#'}];
export default function TodoShow({todo}: {todo: Todo}) {
    ...
    function handleDelete() {
        ...
        router.delete(`/todos/${todo.id}`, {onSuccess: () => router.visit('/todos')});
    }
    function handleSubmit(e: React.SubmitEvent<HTMLFormElement>) {
        e.preventDefault();
        const data = Object.fromEntries(new FormData(e.currentTarget));
        router.patch(`/todos/${todo.id}`, data, {preserveScroll: true, onSuccess: () => setEditing(false)});
    }
    return (
        <>
            <Head title={todo.title}/>
            <div className="p-4 max-w-2xl space-y-6">{/* Header */}
                <div className="flex items-start justify-between">
                    <div className="flex items-start gap-3">
                        <input type="checkbox" checked={todo.completed} onChange={handleToggle} disabled={!isAssigned && !canFullEdit} className="mt-1.5 h-4 w-4 cursor-pointer"/>
                        <div>
                            <h2 className={`text-2xl font-semibold ${todo.completed ? 'line-through text-muted-foreground' : ''}`}>{todo.title}</h2>
                    ...
                    {!editing && (
                        <div className="flex gap-2">
                            {(canFullEdit || isAssigned) && (
                                <button onClick={() => setEditing(true)} className="px-3 py-1.5 border rounded text-sm hover:bg-muted">Edit</button>
                            )}
                            {canDelete && (
                                <button onClick={handleDelete} className="px-3 py-1.5 text-sm text-destructive border border-destructive rounded hover:bg-destructive/10">Delete</button>
                            )}
                        </div>
                    )}
                </div>
                ...
                {editing && (//Edit Form
                    <form onSubmit={handleSubmit} className="rounded-xl border p-4 space-y-3 bg-card">
                        ...
                        <div className="flex gap-2 justify-end">
                            ...
                            <button type="submit" className="px-3 py-1 bg-primary text-primary-foreground rounded text-sm">Save</button>
                        </div>
                    </form>
                )}
            </div>
...
TodoShow.layout = (page: React.ReactNode) => (<AppLayout breadcrumbs={breadcrumbs}>{page}</AppLayout>);