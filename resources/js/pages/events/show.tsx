...
const breadcrumbs: BreadcrumbItem[] = [{title: 'Events', href: '/events'}, {title: 'Event', href: '#'}];
export default function EventShow({event}: {event: Event}) {
    const {auth} = usePage().props;
    const [editing, setEditing] = useState(false);
    const canEdit = auth.user.id === event.user_id || auth.user.status === 'parent';
    function handleDelete() {
        if (!confirm('Delete this event?')) {
            return;
        }
        router.delete(`/events/${event.id}`, {onSuccess: () => router.visit('/events')});
    }
    function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        e.preventDefault();
        const data = Object.fromEntries(new FormData(e.currentTarget));
        router.patch(`/events/${event.id}`, data, {preserveScroll: true, onSuccess: () => setEditing(false)});
    }
    return (
        <>
            ...
            <div className="p-4 max-w-2xl space-y-6">{/* Header */}
                <div className="flex items-start justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">By {event.user.name} (@{event.user.username})</p>
                    </div>
                    {canEdit && !editing && (
...
EventShow.layout = (page: React.ReactNode) => (<AppLayout breadcrumbs={breadcrumbs}>{page}</AppLayout>);